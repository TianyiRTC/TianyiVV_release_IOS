...
- (void)viewDidLoad {
    ...
    if(firstPageLogin&&![self.loginID isEqualToString:@""])
        [self autoLogin];
...
-(void)onSDKInit
{
    ...
    [mSDKObj setSdkAgent:APP_USER_AGENT terminalType:self.terminalType UDID:[OpenUDID value] appID:mAppID.text appKey:mAppKey.text];

    [mSDKObj setDelegate:self];
    int ret;
    ret = [mSDKObj doNavigation:@"default"];
}
...
//                mode = SDK_GROUP_AUDIO_SENDRECV;
    ...
    if (MSG_START_RECORDING == msgid)//开始录像
    {
        ...
        [mCallObj doStartRecording:documentsDirectory];
        return;
    }
...
//        [thePlayer prepareToPlay];
    ...
    if ([self isBackground])
    {
        ...
        if((callType == AUDIO_CALL || callType == AUDIO_CALL_RECVONLY || callType == AUDIO_CALL_SENDONLY))
            callTypeStr = @"语音来电";
        else
            callTypeStr = @"视频来电";
    ...
    if([self.autoAccept isEqualToString:@"YES"])//自动应答
    {
        ...
        NSDictionary* params = [NSDictionary dictionaryWithObjectsAndKeys:@"", @"params",
                                ...
                                [NSNumber numberWithLongLong:(long long)(callingView.remoteVideoView)],@"rvideo",
                                [NSNumber numberWithLongLong:(long long)(callingView.localVideoView)],@"lvideo",
                                nil];
...
#if(SDK_DEBUG_DEMO>0)
        makeNotification(@"查看",[NSString stringWithFormat:@"注册成功"],UILocalNotificationDefaultSoundName,NO);
#endif
        if(firstPageLogin)
        {
            [self presentViewController:self.tabBarController animated:NO completion:nil];
...
#if(SDK_DEBUG_DEMO>0)
        makeNotification(@"查看",[NSString stringWithFormat:@"注册失败"],UILocalNotificationDefaultSoundName,NO);
#endif
    }
...
-(int)onGroupResponse:(NSDictionary*)result grpObj:(CallObj*)grpObj
{
    ...
    if(num)//查询列表触发
    {
        ...
        [[NSNotificationCenter defaultCenter]
         ...
         object:list];//踢出操作2，给麦操作2，获取列表操作2:成员列表发给cantactlist
...
-(void)onGroupRequest:(int)action param:(NSDictionary*)param
{
    // TODO
}
#endif
...
#if(SDK_DEBUG_DEMO>0)
    makeNotification(@"keepAlive",@"keepAliveDo",UILocalNotificationDefaultSoundName,NO);
#endif
...
#if(SDK_DEBUG_DEMO>0)
        makeNotification(@"networkChanged",@"networkChanged",UILocalNotificationDefaultSoundName,NO);
#endif
    if(netstatus)
