...
#define APP_VERSION         @"V2.7.0_B20160720"

#define APPID @"70063"
#define APPKEY @"MTQzMTQyNTM2MzA4NQ=="

//#define PUSHID           @"uHp9HjtwsN9VJa73c1LwTA"
//#define PUSHKEY          @"bhrm1FWNj37grSNpB8eBg2"
//#define PUSHMASTERSECRET       @"vGzFFNnw8a8n16DMieSxM6"

#define DEFAULT_ADDRESS     @"cloud2-70063"
...
@synthesize myLoginListPath/*,pushToken,pushInfo*/;
/**********************************文件管理*************************************/
-(void) write
{
    //创建文件管理器
...
-(void)writeLoginToFile
{
    ...
    self.myLoginListPath = [documentsPath stringByAppendingPathComponent:@"LoginList.plist"];
    //Load the file in a dictionnary
    NSMutableDictionary *dict = [[NSMutableDictionary alloc] initWithContentsOfFile:self.myLoginListPath];
    if (dict == nil) {
        dict = [NSMutableDictionary dictionaryWithCapacity:10];
    }
    [dict setObject:isUserLogin forKey:@"isUserLogin"];
    [dict writeToFile:self.myLoginListPath atomically:YES];
}
...
- (void)viewDidLoad {
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:self.myLoginListPath])
    {
        //创建文件fileName文件名称，contents文件的内容，如果开始没有内容可以设置为nil，attributes文件的属性，初始为nil
        NSData *d_data = [[NSMutableDictionary alloc] init];
        [d_data setValue:@"" forKey:@"LoginID"];//手机号
        [d_data setValue:@"0" forKey:@"isUserLogin"];//是否已登录
        [[NSFileManager defaultManager]  createFileAtPath:self.myLoginListPath contents:d_data attributes:nil];
        [d_data release];
    }
    ...
    isUserLogin = [dict objectForKey:@"isUserLogin"];
    isFirstpage = YES;
    thePlayer = nil;
    mSDKObj = nil;
    mAccObj = nil;
    ...
    //用户在登录状态下退出应用后，下次打开应用则自动登录
    if([isUserLogin isEqualToString:@"1"])
       [self autoLogin];
}
...
- (IBAction)onSetAddress:(id)sender
{
    UIActionSheet* act = [[UIActionSheet alloc]initWithTitle:@"导航地址选择"
                                           ...
                                           cancelButtonTitle:@"取消"
                                      destructiveButtonTitle:nil
                                           otherButtonTitles:@"cloud2-123",@"cloud2-70038",@"cloud2-70063",
                          nil];
        ...
        if (idx >= 0 && idx <= 5)
        {
            switch (idx)
            {
                ...
                case 1:
                    ...
                    [mAppID setText:@"70038"];
                    [mAppKey setText:@"MTQxMDkyMzU1NTI4Ng=="];
                    break;
                case 2:
                    [mNaviAddress setText:@"cloud2-70063"];
                    [mAppID setText:@"70063"];
                    [mAppKey setText:@"MTQzMTQyNTM2MzA4NQ=="];
                    break;
            }
        }
...
- (void)isMuted:(BOOL)muted {
    if (muted) {
        NSLog(@"isDeviceMuted>>>>>>Muted");
    }
    else {
        NSLog(@"isDeviceMuted>>>>>>Not Muted");
...
-(int)onRegisterResponse:(NSDictionary*)result  accObj:(AccObj*)accObj
{
    ...
    if(mToken)
    {
        ...
        else
        {
            ...
            [newResult setObject:mAccountID forKey:KEY_RTCACCOUNTID];
            if(changeVersion==2)
                [newResult setValue:[NSNumber numberWithDouble:2] forKey:KEY_ACC_SRTP];//若与浏览器互通则打开,should be double in arm64，若使用70038测试，请不要加这句话
            [mAccObj doAccRegister:newResult];
        }
        return EC_OK;
    }
...
//            if(pushToken)
//                [mAccObj setAPNsToken:pushToken andPushId:PUSHID andPushKey:PUSHKEY andPushMaster:PUSHMASTERSECRET];
        }
        isUserLogin = @"1";
        [self writeLoginToFile];
        [self writeCurrentInfoToFile];
