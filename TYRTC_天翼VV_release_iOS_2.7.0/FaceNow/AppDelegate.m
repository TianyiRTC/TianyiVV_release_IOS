...
//此应用各界面的层级关系：应用启动时创建navController和viewController，window下加载navController，作为根节点。navController下加载viewController，viewController里管理所有的RTC回调和通话相关事件监听。应用启动时进入viewController，并在viewDidLoad中显示登录界面，同时创建tabBarController，tabBarController下加载三个naviController，分别管理好友、群组和设置。登录成功后present tabBarController。callingview管理来电和通话界面，与tabBarController属于平级关系，来电时present callingview，同时dismiss tabBarController。通话结束后dismiss callingview，同时present tabBarController。不同ViewController之间的消息通过postNotificationName来传递。
